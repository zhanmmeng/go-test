...
func JWTAuth(GuardName string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, err := jwt.ParseWithClaims(tokenStr, &services.CustomClaims{}, func(token *jwt.Token) (interface{}, error) {
			return []byte(conf.Global.ConfigStruct.Jwt.Secret), nil
		})
		...
		// token 续签
		if claims.ExpiresAt-time.Now().Unix() < conf.Global.ConfigStruct.Jwt.RefreshGracePeriod {
			lock := global.Lock("refresh_token_lock", conf.Global.ConfigStruct.Jwt.JwtBlacklistGracePeriod)
			if lock.Get() {
				err, user := services.JwtService.GetUserInfo(GuardName, claims.Id)
				if err != nil {
					conf.Global.Log.Error(err.Error())
					lock.Release()
				} else {
