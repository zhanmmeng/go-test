...
func (jwtService *jwtService) CreateToken(GuardName string, user JwtUser) (tokenData TokenOutPut, err error, token *jwt.Token) {
	token = jwt.NewWithClaims(
		...
		CustomClaims{
			StandardClaims: jwt.StandardClaims{
				ExpiresAt: time.Now().Unix() + conf.Global.ConfigStruct.Jwt.JwtTtl,
				Id:        user.GetUid(),
	...
	tokenStr, err := token.SignedString([]byte(conf.Global.ConfigStruct.Jwt.Secret))

	tokenData = TokenOutPut{
		tokenStr,
		int(conf.Global.ConfigStruct.Jwt.JwtTtl),
		TokenType,
		user.GetUid(),
		user.GetAccount(),
	}
...
func (jwtService *jwtService) JoinBlackList(token *jwt.Token) (err error) {
	...
	err = conf.Global.Redis.SetNX(context.Background(), jwtService.getBlackListKey(token.Raw), nowUnix, timer).Err()
	return
}

// IsInBlacklist token 是否在黑名单中
func (jwtService *jwtService) IsInBlacklist(tokenStr string) bool {
	joinUnixStr, err := conf.Global.Redis.Get(context.Background(), jwtService.getBlackListKey(tokenStr)).Result()
	joinUnix, err := strconv.ParseInt(joinUnixStr, 10, 64)
	if joinUnixStr == "" || err != nil {
		return false
	}
	// JwtBlacklistGracePeriod 为黑名单宽限时间，避免并发请求失效
	if time.Now().Unix()-joinUnix < conf.Global.ConfigStruct.Jwt.JwtBlacklistGracePeriod {
		return false
	}
	return true
}
